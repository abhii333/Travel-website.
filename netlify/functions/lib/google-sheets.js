// Google Sheets integration for Netlify Functions (and anywhere in this repo).
//
// ZERO npm dependencies — uses only Node built-ins (crypto + native fetch).
// This keeps the deployment bundle tiny and the cold-start fast.
//
// Auth flow (RFC 7523 OAuth2 JWT Bearer):
//   1. Build a signed JWT using the service account's private key.
//   2. Exchange the JWT for a short-lived access token at oauth2.googleapis.com.
//   3. Call the Sheets REST API with the bearer token.
// The access token is cached for ~50 minutes (it's valid for 60) to avoid
// re-signing on every request.
//
// On any failure we log a warning and return { ok: false } — the caller
// already saved the booking to Netlify Blobs, so a Sheets outage must NOT
...
let cachedToken = null; // { accessToken, expiryMs }
let cachedCreds = null; // parsed service account JSON
...
async function getAccessToken() {
  // Reuse token until ~10 minutes before expiry to avoid clock-skew surprises.
