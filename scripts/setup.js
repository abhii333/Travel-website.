...
async function step5_testConnection(sheetId, creds) {
  ...
  process.env.GOOGLE_CREDENTIALS_BASE64 = creds.b64;
...
function step6_writeEnv(sheetId, creds, tabName) {
  ...
  const backendEnv =
    ...
    `GOOGLE_CREDENTIALS_BASE64=${creds.b64}\n`;
  ...
  const netlifySnippet =
    `# Paste these into Netlify → Project configuration → Environment variables:\n` +
